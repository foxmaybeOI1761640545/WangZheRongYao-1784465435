...
function deleteServer() {
  ...
  if (!server) return
  const confirmed = window.confirm(`确定删除“${server.serverName}”及其全部资料吗？`)
  if (!confirmed) return
...
function saveGroupName() {
  if (currentGroup.value.id === 'root') return
  store.renameGroup(currentGroup.value.id, settingsForm.name)
}

function deleteCurrentGroup() {
  const group = currentGroup.value
  if (group.id === 'root') return
  const confirmed = window.confirm(`确定删除“${group.name}”以及其中的所有子分组和区服吗？`)
  if (!confirmed) return
...
<template>
  <div class="app-shell">
    ...
    <footer class="app-footer">
      资料仅保存在当前浏览器中。清理站点数据前请确认已不再需要这些记录。
    </footer>
  </div>
  ...
  <div v-if="showGroupDialog" class="modal-backdrop" @click.self="showGroupDialog = false">
    <form class="modal" @submit.prevent="submitGroup">
      <div class="modal-heading">
        <div>
          <span class="section-kicker">New group</span>
          <h3>新建子分组</h3>
        </div>
  ...
  <div v-if="showServerDialog" class="modal-backdrop" @click.self="showServerDialog = false">
    <form class="modal wide" @submit.prevent="submitServer">
      <div class="modal-heading">
        <div>
          <span class="section-kicker">New server</span>
          <h3>添加区服账号</h3>
        </div>
  ...
  <div v-if="showSettingsDialog" class="modal-backdrop" @click.self="showSettingsDialog = false">
    <section class="modal settings-modal">
      <div class="modal-heading">
        <div>
          <span class="section-kicker">Group settings</span>
          <h3>分组设置</h3>
        </div>
      ...
      <div class="order-section">
        ...
        <div v-if="settingsGroups.length" class="order-list">
          <div v-for="item in settingsGroups" :key="item.id" class="order-item">
            <span class="order-kind">分组</span>
            <strong>{{ item.name }}</strong>
            <span class="order-controls">
              <button
                class="mini-button"
                type="button"
                :disabled="!store.canMove(currentGroup.id, item.id, -1)"
                @click="store.moveChild(currentGroup.id, item.id, -1)"
              >↑</button>
              <button
                class="mini-button"
                type="button"
      ...
      <div class="order-section">
        <div class="section-heading compact">
          <div>
            <span class="section-kicker">Two-column cards</span>
            <h3>区服卡片顺序</h3>
          </div>
        </div>
        ...
        <div v-if="settingsServers.length" class="order-list">
          <div v-for="item in settingsServers" :key="item.id" class="order-item">
            <span class="order-kind server">区服</span>
      ...
      <div class="settings-footer">
        <button v-if="currentGroup.id !== 'root'" class="button danger ghost" type="button" @click="deleteCurrentGroup">
          删除当前分组
        </button>
